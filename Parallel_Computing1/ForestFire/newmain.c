...
void main()
{
	...
	for( i=0; i<nTrials; i++ )
	{
		...
		// base step - change every T-node in column to B-node and put node on B-list

		baseStep();

		// printList( BHead, "B-list" );
		...
		do
		{
			// traverse B-list and change any T neighbor to F and add to FList

			done = contStep();

			// in F-list nodes, change F-state to B-state, then connect F-list to B-list
