...
void displayfunc()
{
   ...
   glRasterPos2f(0.85*N,0.9*N); //DUNNO IF I NEED THIS OR NOT
   conv( step , stepstr ) ;
   drawString(stepstr);
   //
...
void mousefunc(int button,int state,int xscr,int yscr)
{
   if(button==GLUT_LEFT_BUTTON)
   {
      if(state==GLUT_DOWN)
      {
         init( prob ) ; //change viewport window to divided by 2
         fire( ) ; //displayfunc(mandelbrotset);
      }
   }
   else if(button==GLUT_RIGHT_BUTTON)
   {
      if(state==GLUT_DOWN)//change viewport window to times by 2
         fire( ) ; //displayfunc(mandelbrotset);
      {
         srand( time(NULL) ) ;
      }
